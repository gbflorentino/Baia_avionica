...
void ServoMotor::ParachuteSignal(ServoMotor& s)
{
    s.servo_signal = 180;
    LOGN("");
    s.main_servo.write(0);
    delay(4000);
    s.main_servo.write(90);
    s.redundant_servo.write(s.servo_signal);
    delay(3000);
    s.redundant_servo.write(90);
    delay(50);
}

void ServoMotor::SetMotor(Servo& s_object, unsigned short sp)
{
  if ((sp <=180 && sp >= 0) || (sp >= -180 && sp <= 0))
    s_object.write(sp);
}

void ServoMotor::Init(ServoMotor& s_object)
{ 
  s_object.main_servo.attach(SERVO_1);
  s_object.redundant_servo.attach(SERVO_2);
  LOGN("::::::The ServoMotor Class was succefuly initialized::::::");
}
